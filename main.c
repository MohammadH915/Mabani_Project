...
void handleEvent(SDL_Event* e) {
	...
	if(Inside_Box(e, 0, 0, 70, 30) == BUTTON_SPRITE_MOUSE_DOWN) {
		MENU();
	}
	int cnt = 1, cnn = 1;
...
void JMap_Render() {
	UPDATE_LOCATION();
	boxRGBA(gRenderer, 0, 0, 70, 30, 0, 0, 0, 150);
	char menu[5] = "MENU\0";
	SDL_Color color2 = { 255, 255, 255 };
	Print(menu, 0, 0, 10, 10, color2);
	if(Nobat()) {
		boxRGBA(gRenderer, 100, 0, 70, 30, 102, 94, 76, 150);
		char m[16]= "Detective Turn\0";
		SDL_Color color2 = { 128, 255, 255 };
		Print(m, 100, 0, 10, 10, color2);
	}
	else {
		boxRGBA(gRenderer, 100, 0, 70, 30, 102, 94, 76, 150);
		char m[11]= "Jack Turn\0";
		SDL_Color color2 = { 128, 255, 255 };
		Print(m, 100, 0, 10, 10, color2);
	}
...
int main() {
	...
	SHOW_JACK();
	while (BUTTON_END == false && LEVEL_NUM < 9) {
  		SDL_SetRenderDrawColor(gRenderer, 102, 94, 76, 0);
  		SDL_RenderClear(gRenderer);
  		SDL_Event sdlEvent;
  		int SHUTDOWNCODE=0;
